...
function sign(role: "buyer" | "vendor", signer: Keypair) {
  return { role, publicKey: signer.publicKey.toBase58(), signature: bs58.encode(nacl.sign.detached(new TextEncoder().encode(approvalMessage(terms, role)), signer.secretKey)) };
}
...
test("creates a portable receipt with valid dual approvals", () => {
  ...
  const receipt = createReceipt(terms, approvals, "x".repeat(88));
