...
function sign(role: "buyer" | "vendor", signer: Keypair) {
  const message = new TextEncoder().encode(approvalMessage(terms, role));
...
test("verifies approval against role and exact terms", () => {
  ...
  assert.equal(verifyWalletApproval(terms, approval), true);
  assert.equal(verifyWalletApproval({ ...terms, annualPrice: 43201 }, approval), false);
  assert.equal(verifyWalletApproval(terms, { ...approval, role: "vendor" }), false);
});

test("requires distinct buyer and vendor wallets", () => {
  const buyer = Keypair.generate();
  const vendor = Keypair.generate();
  assert.equal(verifyDualApprovals(terms, [sign("buyer", buyer), sign("vendor", vendor)]).valid, true);
  assert.equal(verifyDualApprovals(terms, [sign("buyer", buyer)]).valid, false);
  assert.equal(verifyDualApprovals(terms, [sign("buyer", buyer), sign("vendor", buyer)]).valid, false);
});
