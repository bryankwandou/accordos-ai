...
import { Check, ExternalLink, Fingerprint, LoaderCircle, PlugZap, ShieldCheck, Wallet } from "lucide-react";
...
export function WalletProof({ terms, ready }: { terms: Record<string, TermValue>; ready: boolean }) {
  ...
  async function connect() {
    ...
    if (!wallet) {
      setError("Install Phantom or Solflare, then reload this page.");
      return;
    }
    try {
  ...
  async function anchor() {
    ...
    try {
      const response = await fetch("/api/proof", {
        ...
        body: JSON.stringify({ terms, approvals: 2, walletAddress: address }),
      });
      ...
      const verificationResponse = await fetch("/api/proof/verify", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ signature: signed.signature, hash: prepared.hash }),
      });
      const verification = await verificationResponse.json();
      setProof({ hash: prepared.hash, signature: signed.signature, verified: verification.verified, slot: verification.slot });
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Devnet transaction failed");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <motion.div className="wallet-proof" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
      {!address ? (
        <button className="wallet-connect" onClick={connect}><Wallet size={16} /><span><b>Connect Solana wallet</b><small>Phantom or Solflare · Devnet</small></span><PlugZap size={15} /></button>
      ) : (
        <div className="wallet-connected"><span><i /><b>{address.slice(0, 4)}…{address.slice(-4)}</b><small>{balance?.toFixed(4) ?? "…"} DEVNET SOL</small></span><ShieldCheck size={17} /></div>
      )}
      {address && !proof && <button className="button proof-button" disabled={!ready || busy || balance === 0} onClick={anchor}>{busy ? <LoaderCircle className="spin" size={16} /> : <Fingerprint size={16} />}{busy ? "Signing and verifying…" : "Sign devnet proof"}</button>}
